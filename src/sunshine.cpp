...
std::shared_ptr<SunShine> SunShine::addSunshineChild(std::shared_ptr<Object> parent, glm::vec2 pos, glm::vec2 dest)
{
    std::shared_ptr<SunShine> sunshine = std::make_shared<SunShine>();
    sunshine->setPos(pos);
...
void SunShine::update(float dt)
{
    ...
    if (glm::distance(pos_, dest_) < speed_ * dt)
    {
        pos_ = dest_;
        speed_ = 0.0;
        if (dest_ == std::dynamic_pointer_cast<SceneMain>(parent_)->getSunshineCollectorPos())
        {
            is_collected_ = true;
        }
    }
...
void SunShine::clicked(SDL_Event &event)
{
    if (event.type == SDL_MOUSEBUTTONDOWN && event.button.button == SDL_BUTTON_LEFT)
    {
        if (glm::distance(glm::vec2(event.button.x, event.button.y), pos_) < width_ / 2.0f && !clicked_)
        {
            ...
            // 播放音效
            if (!has_sound_){
                auto parent = std::dynamic_pointer_cast<SceneMain>(this->parent_);
